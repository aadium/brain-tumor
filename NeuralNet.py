...
class NeuralNetModel:
    ...
    def trainSLNN(self, X_train: np.ndarray, Y_train: np.ndarray, X_val: np.ndarray, Y_val: np.ndarray, Layer1_neurons: int, num_epochs: int, lr: float):
        (N_train, M), C = X_train.shape, Y_train.shape[1]
        W1 = np.random.uniform(high=-0.1, low=0.1, size=(Layer1_neurons, M+1))
        W2 = np.random.uniform(high=-0.1, low=0.1, size=(C, Layer1_neurons+1))
    ...
    def trainDLNN(self, X_train: np.ndarray, Y_train: np.ndarray, X_val: np.ndarray, Y_val: np.ndarray, Layer1_neurons: int, Layer2_neurons: int, num_epochs: int, lr: float):
        (N_train, M), C = X_train.shape, Y_train.shape[1]
        W1 = np.random.uniform(high=-0.1, low=0.1, size=(Layer1_neurons, M+1))
        W2 = np.random.uniform(high=-0.1, low=0.1, size=(Layer2_neurons, Layer1_neurons+1))
        W3 = np.random.uniform(high=-0.1, low=0.1, size=(C, Layer2_neurons+1))
    ...
    def predictSingle(self, X: np.ndarray):
        N, M = X.shape
        yHat = []
        for i in range(N):
            Xi = X[i, :].reshape((M, 1))
            neural = self.fwd.SLNN(Xi, self.model['W1'], self.model['W2'])
    ...
    def predictDouble(self, X: np.ndarray):
        N, M = X.shape
        yHat = []
        for i in range(N):
            Xi = X[i, :].reshape((M, 1))
            neural = self.fwd.DLNN(Xi, self.model['W1'], self.model['W2'], self.model['W3'])
            Yi = np.argmax(neural['y_prob'], axis=0)
            yHat.append(Yi)
